...
class PhoneValidationError(Exception):
    def __init__(
        self,
        message: str = ErrorMessageEnum.PHONE_LENGTH.value
    ) -> None:
...
class NameValidationError(Exception):
    def __init__(
        self,
        message: str = ErrorMessageEnum.NAME_LENGTH.value
    ) -> None:
...
class BirthdayValidationError(Exception):
    def __init__(
        self,
        message: str = ErrorMessageEnum.BIRTHDAY_FORMAT.value
    ) -> None:
...
class ItemNotFoundError(Exception):
    def __init__(
        self,
        message: str = ErrorMessageEnum.ITEM_NOT_FOUND.value,
        item: str = ""
    ) -> None:
        ...
        def inner(*args, **kwargs):
            try:
                ...
                return "Give right arguments, please.\n" +\
                    ...
                    "\tPhone format: all characters in a string " +\
                    "are digits, length equal 10"
            except KeyError:
